...
In a CNN the image size is reduced by convolutional and pooling layers until 
there is only a one-dimensional feature vector which is then fed into a "normal"
MLP.

### A simplified view on CNNs

I find it easier to think of CNNs as network of units that input and output 
whole images or feature maps. So in this view there is just a single input unit.
In a convolutional layer each unit receives a number of connections from the 
previous layer. For each connection there is a two-dimensional kernel 
(or filter) that is used to convolve the image that goes through this connection
to the unit. All the convolved images are summed by the unit, a bias is added 
and an activation function applied one each pixel.
