...
Although I knew how to implement an "ordinary" Multilayer Perceptron (MLP, the 
"normal" neural network), I got very confused when trying to implement a CNN. In
the end I wanted to fully understand the math to understand how to get a working
implementation.
__Isn't this just backpropagation as usual?__
Well, basically yes, but a bit more complex and with some pitfalls, so read on.

### The idea of CNNs, in short

Using a "normal" MLP does not work well for images, because images have many
pixels which would result in many many weights to train. Furthermore, these nets
would not make use of the inherent property of images that information is local,
rather than distributed over the full image.
...
the number of trainable weights. The weight sharing can be seen as convolution 
where a filter is applied on an image. This also makes more sense for data like
images becuase two-dimensional features are extracted. To speed up things even
more, usually each convolutional layer is followed by a pooling layer, that just
reduces the size of intermediary images (feature maps).
In a CNN the image size is reduced by convolutional and pooling layers. 
Finally, at some point the data is squashed into a one-dimensional feature 
vector which is then fed into a "normal" MLP.

### A simplified view on CNNs

I find it easier to think of CNNs as network of units that input and output 
whole images or feature maps. In this view there is just a single input unit, 
which is just the input image. Following layers, usually an alternation of 
convolutional and pooling layers, then consist of an arbitrary number of units.
...
kernel (or filter) that is used to convolve the image that is output from a 
unit of the previous layer to the unit. All the convolved images are summed by 
the unit, a bias is added and usually but not always an activation function is 
applied one each pixel.
...
<div class="float-container">
  <div id="float-0" style="float: left; width: 28em; border-right: thin solid #ddd; pading-right: 1em">
    <h4>Convolution</h4>
    ...
    <div style="float: left; height: 100%; border-left: thin solid #ddd;"></div>
  <div id="float-1" style="float: left; width: 28em; padding-left: 1em;">
